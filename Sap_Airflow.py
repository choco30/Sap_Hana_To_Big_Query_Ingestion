...
start_dataflow_job=BashOperator(
    ...
    bash_command='''job_date=$(date '+%Y-%m-%d-%H-%M-%S'); python3 /home/airflow/gcs/dags/dataflow_template.py --runner DataflowRunner --project project_name --region asia-south1 --temp_location bucket/temp --network network_name  --max_num_workers 5 --job_name {5}-$job_date --worker_machine_type {7} --service_account_email  service_account-name --setup_file /home/airflow/gcs/dags/setup.py --df_bucket bucket_name '''
    dag=dag)
